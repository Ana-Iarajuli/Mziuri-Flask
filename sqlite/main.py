...
#     print(row)
ragac = []
purchase_date = cursor.execute('SELECT purchase_date FROM purchase').fetchall()
for date in purchase_date:
    date_list = date[0].split("-")
    if int(date_list[0]) < 2018:
        ragac.append(date)
        bookid = cursor.executemany('SELECT book_id FROM purchase').fetchall()
